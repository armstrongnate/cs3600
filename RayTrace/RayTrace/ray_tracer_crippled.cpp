...
void Scene::CalculateRightAndUp()
{
    ...
    // first find u
    double u = tan(fov*3.1415/180/2.0);
    // next find r
    //(screen_x/screen_y)=(r/u) solve for r
    double r = (double)screen_x/screen_y*u;

    ww_tr = eye + forward + right*r + up*u;
    ww_tl = eye + forward - right*r + up*u;
    ww_br = eye + forward + right*r - up*u;
    ww_bl = eye + forward - right*r - up*u;
}
...
void Scene::CreateRay(int x, int y, Vector3 &ray)
{
	...
	Point3 bottom_edge = AffineSum(ww_bl, ww_br, xRatio);
	Point3 top_edge = AffineSum(ww_tl, ww_tr, xRatio);
	Point3 F = AffineSum(bottom_edge, top_edge, yRatio);
...
bool RayIntersectSphere(const Point3 &eye,
						...
						const Sphere & s,
						float & distance,
						Point3 &intersection_point, // = u+d*t
						Vector3 &intersection_normal // interesection_point - s normalized
)
{
    ////// Draw Sphere //////
    //x = origin.x + direction.dx * t
    //p(t) = u + d*t
    //(x-Sx)^2 + (y-Sy)^2 + (z-Sz)^2 - R^2 = 0

    //(ux + dx*t-Sx)^2 + (uy + dy*t - Sy)^2 + (uz + dz*t - Sz)^2 -R^2 = 0
    //mx = ux-sx
    //-> (mx+dx*t)^2
    // = (mx^2 + 2mx*dx*t + dx^2*t^2) + ( ) + ( ) - R^2 = 0

    //use quadratic formula to find t
    // t = (-B +|- sqrt(B^2-4AC))/2A
    //A = dx^2 + dy^2 + dz^2 or A = DotProduct(d,d)
    //B = 2mx*dx + 2mx*dy + 2mz*dz or B = 2(DotProduct(m,d)
    //C = mx^2 + my^2 + mz^2 - R^2 or C = DotProduct(m,m)-R^2
    // choose the smallest positive t
    double mx = eye.p[0] - s.origin.p[0];
    double my = eye.p[1] - s.origin.p[1];
    double mz = eye.p[2] - s.origin.p[2];
    double r = s.radius;
    ...
    double b = 2*xd*mx + 2*yd*my + 2*zd*mz;
    double c = pow(mx,2) + pow(my,2) + pow(mz,2) - pow(r,2);
...
void Scene::CastRay(const Vector3 &ray,float &r,float &g,float &b)
{
	...
	if(closest_hit_sphere != -1)
	{
        ...
        //diffuse//
        //N = closest normal//
        //D = DotProduct(N, ToLightVector)
        //if D>0
        //  r += sphere[closest_hit_sphere].diffuse[0]*D
        //specular//
        //find normalized sum vector //
        //Sum is to light + to eye
        //D2 = DotProduct(N, Sum)
        //if D2 > 0
        //  D2 = Pow(D2, spheres[closest_hit_sphere].specularExponent);
        //  r += spheres[closest_hit_sphere].specular[0]*D2;
        // cap r, g, and b at 1;
        ...
        for(int i = 0; i < num_point_lights; i++)
        {
            ...
            double D = DotProduct(closest_normal, toLight);
            if(D > 0)
            {
            ...
            Vector3 sum = (toLight + toEye) * 0.5;
            sum.Normalize();
            specular = DotProduct(closest_normal, sum);



            if(specular > 0)
            {
                specular = pow(specular, s[closest_hit_sphere].specular_exponent);
                r += specular * s[closest_hit_sphere].specular[0];
                g += specular * s[closest_hit_sphere].specular[1];
                b += specular * s[closest_hit_sphere].specular[2];
            }
        }
        if(r > 1.0)
            r = 1.0;
...
{
	if(!the_scene.Load("/Users/nate/school/3600/RayTrace/RayTrace/mickey.txt"))
	{
		exit(1);
	}
